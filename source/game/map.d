...
public:
    ...
    Chunk* getChunkPointerMutable(Vec2i key) {
        return key in database;
    }

    Chunk* getChunkPointerMutable(const int keyX, const int keyZ) {
        return Vec2i(keyX, keyZ) in database;
    }
    ...
    void setBlockAtWorldPositionByID(Vec3d position, uint blockID) {
        if (!BlockDatabase.hasBlockID(blockID)) {
            throw new Error("Cannot set to block ID " ~ to!string(
                    blockID) ~ ", ID does not exist.");
        }
        ...
        Light.cascadeNaturalLight(cast(int) floor(position.x), cast(int) floor(
                position.z));
    ...
    void updateHeightMap(Chunk* thisChunk, int xInChunk, int yInChunk, int zInChunk, int newID,
        ...
        if (newID == 0) {
            ...
            return Light.cascadeNaturalLight(worldPositionX, worldPositionZ);
        ...
        foreach (double xOnRect; 0 .. ceil(entitySize.x) + 1) {
            ...
            foreach (double zOnRect; 0 .. ceil(entitySize.x) + 1) {
                ...
                foreach (
                    ...
                    BlockData data = getBlockAtWorldPosition(cast(int) floor(thisXPoint), cast(
                            int) floor(
